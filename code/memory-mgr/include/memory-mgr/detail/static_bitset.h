...
class static_bitset : public detail::array<BlockType, BitsCount, StaticArr>
{
  ...
  */
  ...
  // Default constructor
  // Resets all bits
  inline static_bitset() { static_assert(num_bits != 0, "Bitset cant be empty"); }

  // Constructor used only by custom arrays to initialize
  // array's pointer
  template<typename U = block_ptr_type>
  inline explicit static_bitset(block_ptr_type bits_ptr, std::enable_if_t<std::is_constructible<base_type, U>::value>* = 0) : base_type(bits_ptr) { static_assert(num_bits != 0, "Bitset cant be empty"); }
